...
class SuperAdminProtectionListener
{
        ...
        if ($existingSuperAdmin) {
            ...
            if ($count > 0) {
                throw new \RuntimeException(
                    'Super admin protection: Only one super admin is allowed. ' .
                    'Cannot create additional super admin users after deployment.'
                );
            }
        }
    }
    ...
    public function preRemove(User $user, PreRemoveEventArgs $event): void
    {
        if ($this->hasSuperAdminRole($user)) {
            throw new \RuntimeException(
                'Super admin protection: Cannot delete super admin user. ' .
                'This action is forbidden for security reasons.'
            );
        }
    }
